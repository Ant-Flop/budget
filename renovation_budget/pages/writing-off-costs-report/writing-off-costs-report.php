...
function GetActsData($data)
{
    ...
    $result = $mysql->query("SELECT b.act_number, b.act_date, sum((b.price_of_service_no_pdv + b.cost_of_materials_no_pdv) * b.amount) as sum FROM renovation_treaty_directory a
                             ...
                             LEFT JOIN renovation_act_card rac ON rac.id = b.renovation_act_card_id
                             WHERE rac.conducted = 1 and  a.counterparties_directory_id = '" . $data['counterpartyId'] . "' and a.contracts_directory_id = '" . $data['contractId'] . "' and 
                                   a.article_name = '" . mysqli_real_escape_string($mysql, $data['articleName']) . "' and 
...
function GetServiceData($fundholder_id)
{
    ...
    $mysql->set_charset("utf8");
    $data_array = array();
    $result = $mysql->query("SELECT * FROM services_directory WHERE id = 10 or id = 11"); // fundholders_directory_id = '" . $fundholder_id . "'
    while ($row = $result->fetch_assoc()) {
        array_push($data_array, [
            "id" => $row["id"],
            "name" => $row["name"],
...
function GetBudgetArticleData($data)
{
    ...
    $result = $mysql->query("SELECT a.id, a.article_name as name, c.id as counterparty_id, c.name as counterparty_name  FROM planned_indicators a
                             ...
                             WHERE b.services_directory_id = '" . $data['serviceId'] . "' and 
                             a.year = '" . $data['year'] . "' and 
                             a.counterparties_directory_id != 0 and 
                             (b.subsections_directory_id = 51 or b.subsections_directory_id = 53)");
...
function getContractData($data)
{
    ...
    $mysql->set_charset("utf8");
    $data_array = array();
    $result = $mysql->query("SELECT DISTINCT(contract_number), contracts_directory_id, counterparties_directory_id, planned_indicators_id  
                             FROM renovation_treaty_directory 
                             WHERE counterparties_directory_id = '" . $data['counterpartyId'] . "' and 
                                   article_name = '" . mysqli_real_escape_string($mysql, $data['article_name']) . "'");
    while ($row = $result->fetch_assoc()) {
        array_push($data_array, [
